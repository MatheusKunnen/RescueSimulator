...
class View:
    ...
    def draw(self):
        ...
        if self.posRob != None:
            self.board.listPlaces[self.posRob[0]
                                  ][self.posRob[1]].setAgent(False)
        ...
        self.board.listPlaces[self.model.agentPos[0]
                              ][self.model.agentPos[1]].show()
        self.posRob = (self.model.agentPos[0], self.model.agentPos[1])

        # Desenha a fala do robo na lateral
        # txt = "Estou em (x, y): " + str(self.model.agentPos[1]) + ", " + str(
        #     self.model.agentPos[0]) + " Cambio..."
        # fonte = pygame.font.SysFont(
        #     "Times New Roman", 20, False, False)  # usa a fonte padr�o
        # # renderiza o texto na cor desejada
        # txttela = fonte.render(txt, 0, (0, 0, 0))
