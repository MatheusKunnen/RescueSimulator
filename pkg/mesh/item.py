...
class Item:
    def __init__(self, text, img, size, pos, screen, action=False):
        ...
        self.image = pygame.image.load(
            os.path.join("pkg", "mesh", "images", img)
        ).convert_alpha()
    ...
    def checkClick(self, posMouse):
        if (
            posMouse[0] < self.pos[0]
            or posMouse[0] > self.pos[0] + self.size[0]
            or posMouse[1] < self.pos[1]
            or posMouse[1] > self.pos[1] + self.size[1] + 40
        ):
            return False
        else:
